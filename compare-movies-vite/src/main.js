...
const handleSettings = () => {
  if (!seClicked) {
    ...
    document.getElementById("restoreDefault").style.transform =
      "translateY(0px)";
...
const getMovie = async (event) => {
  ...
  if (jsonResponse["Response"] === "True") {
    ...
    if (jsonResponse["Ratings"].length > 1) {
      document.getElementById("critic").value = jsonResponse["Ratings"][1][
        "Value"
      ].replace(/%/g, "");
      document.getElementById("audience").value = parseFloat(
    ...
    notFound.innerHTML =
      "Movie not found. Check spelling or insert your own data";
  ...
  const addMovie = async (event) => {
    ...
    const cardStore = {
      ...
      box: parseFloat(
        sanitizeInput(document.getElementById("box").value).replace(
          /[^0-9.-]+/g,
          ""
        )
      ),
    ...
    movieCard.innerHTML = `<div id=${Math.floor(
...
<img src = ${localStorage.getItem(
      "Image"
    )} width='100%' height='100%' class='cardPicture'>
`;
...
const renderDef = async () => {
  ...
  defaults.forEach((card) => {
    ...
    movieCard.innerHTML = `<div class='cardInfo'>
...
const loadAdd = async () => {
  ...
  history.forEach((card) => {
    ...
    movieCard.innerHTML = `<div class='cardInfo'>
    <p class='titleVal' style='font-weight:bold; padding-top: 10px'>${
      card.title
    }</p>
    <p class='criticVal'>Critic-score: ${card.critic}%</p>
    <p class='veiwerVal'>Viewer-score: ${card.veiwer}%</p>
...
document.getElementById("dro").addEventListener("click", () => {
  if (!dropActive) {
    ...
    document.getElementById("aa").style.transform = "TR";
  } else if (dropActive) {
    dropActive = false;
